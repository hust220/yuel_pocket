...
def find_pocket_centers(structure: Structure, pocket_pred: np.ndarray, prob_threshold: float = 0.03,
    ...
    top_points = positive_points[sorted_indices]
    top_scores1 = positive_scores1[sorted_indices]
    top_scores2 = positive_scores2[sorted_indices]
    
    # Use Gaussian Mixture Model for clustering
    from sklearn.mixture import GaussianMixture
    
    # Try different numbers of components and select best by BIC
    n_components_range = range(3, 20)  # Increase minimum clusters and range
    bic = []
    models = []
    ...
    for n_components in n_components_range:
        gmm = GaussianMixture(
            ...
            covariance_type='tied',  # Use tied covariance to make clusters more uniform
            random_state=42,
            max_iter=100,
            reg_covar=1e-3  # Add small regularization to prevent singularity
        )
        gmm.fit(top_points)
        # Modify BIC score to prefer more clusters
        score = gmm.bic(top_points) * (1 + 0.01 * (15 - n_components))  # Small penalty for fewer clusters
        bic.append(score)
        models.append(gmm)
    
    # Select best model by BIC
    best_model = models[np.argmin(bic)]
    cluster_labels = best_model.predict(top_points)
    ...
    for cluster_id in unique_clusters:
        ...
        if len(cluster_points) > 0:
            # Use spatial mean as cluster center
