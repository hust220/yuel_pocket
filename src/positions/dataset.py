...
def build_graph_edges(protein_contacts, protein_backbone, mol_bonds, p_size, m_size, prot_pos, probes):
    ...
    # Edge features: [dist, backbone_neighbor, ...bond_one_hot]
    # Total dim: 2 + n_bond_feats
    
    edges = []  # List of records: (u, v, attr_list)

    # 1. Protein-Protein Contacts: [dist, 0, 0, 0, ...0]
    for u, v, dist in protein_contacts:
        edges.append((int(u)-1, int(v)-1, [dist, 0] + [0] * n_bond_feats))
    ...
    for u, v in protein_backbone:
        ...
        if (u, v) in existing_edges:
            ...
            edges.append((u, v, [0, 1] + [0] * n_bond_feats))

    # 3. Protein-Joint Edges: [0, 0, ...0]
    # Joint node index is exactly `p_size`
    joint_idx = p_size
    for i in range(p_size):
        edges.append((i, joint_idx, [0, 0] + [0] * n_bond_feats))

    # 4. Joint-Ligand Edges: [0, 0, ...0]
    # Ligand nodes start at `p_size + 1`
    for i in range(m_size):
        lig_idx = p_size + 1 + i
        edges.append((joint_idx, lig_idx, [0, 0] + [0] * n_bond_feats))
    ...
    if len(mol_bonds) > 0:
        for bond in mol_bonds:
            ...
            feats = bond[2:].tolist()
            edges.append((u, v, [0, 0] + feats))

    # 6. Probe-Protein Edges: [dist, 0, 0, 0, ...0]
        ...
        for i, neighbors in enumerate(queries):
            ...
            if len(neighbors) > 0:
                ...
                for p_idx, dist in zip(neighbors, dists):
                    edges.append((int(p_idx), probe_node_idx, [dist, 0] + [0] * n_bond_feats))
...
def build_graph(protein_name, ligand_name, mol_pos, mol_h, mol_bonds, prot_pos, prot_h, prot_contacts, prot_backbone, sas_points, pick_samples=True):
    ...
    if pick_samples and p_size > 1000: return None # Skip large proteins during training
    ...
    if pick_samples and len(mol_pos) > 0 and sas_points is not None and len(sas_points) > 0:
        ...
        dists = cdist(sas_points, joint_x_raw).min(axis=1)
    ...
    edge_index, edge_attr, complex_mask, probe_mask = build_graph_edges(
        prot_contacts, prot_backbone, mol_bonds, p_size, m_size, 
