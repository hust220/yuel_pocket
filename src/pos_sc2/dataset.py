...
def distance_expansion(dist, num_bins=20):
    centers = np.linspace(0, 10, num_bins)
    sigma = 10.0 / num_bins
    dist = np.array(dist)
    if dist.ndim == 0:
        dist = dist.reshape(1)
    expansion = np.exp(-0.5 * ((dist[:, None] - centers[None, :]) / sigma)**2)
    return expansion.flatten().tolist()

def build_graph_edges(protein_contacts, mol_bonds, mol_bonds2, p_size, m1_size, m2_size, prot_pos, mol_pos, probes, pick_samples=True, n_sas_orig=None):
    ...
    # 1. Protein-Protein Contacts: Bidirectional [dist_expanded, 0]
    for u, v, dist in protein_contacts:
        feat = distance_expansion(dist, num_bins=num_bins) + [0]
        edges.append((int(u), int(v), feat))
        edges.append((int(v), int(u), feat))

    # 2. Ligand 1 Internal Bonds: Bidirectional [0_expanded, 1]
    bond_dist_feat = distance_expansion(0, num_bins=num_bins)
    if len(mol_bonds) > 0:
        for bond in mol_bonds:
            u, v = int(bond[0]) + p_size, int(bond[1]) + p_size
            feat = bond_dist_feat + [1]
            edges.append((u, v, feat))
            edges.append((v, u, feat))
            
    # 2b. Ligand 2 Internal Bonds: Bidirectional [0_expanded, 1]
    if len(mol_bonds2) > 0:
    ...
    # 3. Probe-Protein & Probe-Ligand Edges: [dist_expanded, 0]
    ...
    if len(probes) > 0:
        ...
        # Probe-Protein: All probes connect to protein within 10A
        ...
        for i in range(len(probes)):
            ...
            for p_idx in queries_prot[i]:
                ...
                feat = distance_expansion(dist, num_bins=num_bins) + [0]
                edges.append((int(p_idx), probe_node_idx, feat))

        # Probe-Ligand connections
        zero_dist_feat = distance_expansion(0, num_bins=num_bins)
        if pick_samples and n_sas_orig is not None:
            # Training Mode: Specific connectivity for contrastive learning
            # 1. Original SAS points connect to Ligand 1
            for i in range(n_sas_orig):
                probe_node_idx = probe_start + i
                feat = zero_dist_feat + [0]
                for m_idx in range(m1_size):
                    edges.append((p_size + m_idx, probe_node_idx, feat))
            
            # 2. Duplicate pocket points (starting from n_sas_orig) connect to Ligand 2
            for i in range(n_sas_orig, len(probes)):
                probe_node_idx = probe_start + i
                feat = zero_dist_feat + [0]
                for m_idx in range(m2_size):
                    edges.append((p_size + m1_size + m_idx, probe_node_idx, feat))
        else:
            # Inference Mode or Fallback: All probes connect to Ligand 1
            for i in range(len(probes)):
                probe_node_idx = probe_start + i
                feat = zero_dist_feat + [0]
                for m_idx in range(m1_size):
...
def build_graph(protein_name, ligand_name, mol_pos, mol_h, mol_bonds, prot_pos, prot_h, prot_contacts, sas_points, mol_h2=None, mol_bonds2=None, pick_samples=True):
    ...
    if pick_samples and len(mol_pos) > 0 and sas_points is not None and len(sas_points) > 0:
        # Training Mode: Pick one pocket and one decoy
        ...
        # selected_probes: [Pocket, Decoy, Pocket (Duplicated for Ligand 2)]
    ...
    if is_training_sample and n_probes > 0:
        # 1. Pockets (Original)
        is_pocket[probe_start : probe_start + n_pockets] = 1.0
        # 2. Decoys (10 sampled)
        is_decoy[probe_start + n_pockets : probe_start + n_sas_orig] = 1.0
        # 3. Duplicate pocket points (all points after n_sas_orig)
        is_decoy2[probe_start + n_sas_orig:] = 1.0

        # print(is_pocket.sum(), is_decoy.sum(), is_decoy2.sum())
    ...
    edge_index, edge_attr, complex_mask, probe_mask = build_graph_edges(
        ...
        n_sas_orig=n_sas_orig if is_training_sample else None
    )
