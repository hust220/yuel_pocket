...
def evaluate_predictions(dataset_folder, yuel_dir=None, mode="pos_aa3"):
    ...
    for lig_file in ligand_files:
        # Determine system_id
        # Holo4k: 1a0j_BEN_ligand.pdb -> PDBID is 1a0j
        # Plinder: 1afb__1__1.A__1.D_1.F_ligand.sdf -> PDBID system is 1afb__1__1.A__1.D_1.F
        
        filename = lig_file.name
        
        # Try to guess format
        if len(filename.split('_')) >= 2 and len(filename.split('_')[0]) == 4:
             # Holo4k likely: 1a0j_BEN_ligand.pdb or 1a0j_protein.pdb
             # But evaluate iterates over ligands.
             # Taking first 4 chars is risky if Plinder has 4 char prefixes.
             # But Holo4k is consistently 4 char PDB ID.
             # Plinder system IDs are long.
             # Let's try matching protein file exists.
             ...
             if (dataset_dir / f"{candidate_pdb_id}_protein.pdb").exists():
                 ...
                 # Fallback to previous logic (Plinder style?)
                  if "_ligand" in filename:
                        system_id = filename.split('_ligand')[0]
                  else:
                        system_id = lig_file.stem
        else:
             if "_ligand" in filename:
                system_id = filename.split('_ligand')[0]
             else:
