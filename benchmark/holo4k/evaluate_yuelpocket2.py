...
# Assuming this script is at benchmark/holo4k/evaluate_yuelpocket2.py, 
# project root is ../../
...
def evaluate_residues_and_pockets(dataset_folder, yuel_dir=None):
    ...
    if not ligand_files:
        # Holo4k/Coach structure might be slightly different or ligand naming
        # Fallback search
    ...
    for lig_file in ligand_files:
        ...
        # Determine system_id (protein part) from ligand filename
        if "_ligand" in lig_file.name:
            # Handle formats like 1a0j_BEN_ligand.pdb -> matches 1a0j_protein.pdb
            # or 148lE_MUB_ligand.pdb -> 148lE_protein.pdb
            # Try prefixes
            parts = lig_file.name.split('_')
            # We assume the filename ends with _ligand.pdb or _ligand.sdf
            # parts will be ['1a0j', 'BEN', 'ligand.pdb']
            
            # Try to find a matching protein file by joining parts
            for i in range(1, len(parts)):
                candidate = "_".join(parts[:i])
                candidate_prot = dataset_dir / f"{candidate}_protein.pdb"
                if candidate_prot.exists():
                    system_id = candidate
                    prot_file = candidate_prot
                    break
            
            # If heuristic failed, fallback to original logic or just splitting off _ligand
        ...
        # Predictions path
        # Prediction files are named {system_id}_predictions... based on the protein filename usually
        pred_base = yuel_dir / f"{system_id}_predictions"
        pred_txt = Path(str(pred_base) + ".txt")
        cluster_csv = Path(str(pred_base) + "_clusters.csv")
        
        # If standard protein file not found, try just .pdb (excluding ligand)
        ...
        try:
            ...
            # 2. Identify ALL pocket residues
...
if __name__ == "__main__":
    ...
    parser.add_argument("dataset_folder", nargs="?", default="random50", help="Dataset folder")
