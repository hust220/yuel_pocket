...
def load_ligand_data(lig_file):
    """Load ligand atom coordinates and center."""
...
def load_yuelpocket_clusters(csv_path):
    """Load cluster centers from _clusters.csv."""
    if not os.path.exists(csv_path):
        return []
    try:
        df = pd.read_csv(csv_path)
        if df.empty: return []
        # Group by ClusterID and take mean of X, Y, Z for center
        # Also sum the scores
        clusters = []
...
def evaluate_residues_and_pockets(dataset_folder, yuel_dir=None):
    ...
    # Residue Discovery data: list of systems, each system is a list of (min_dist, score)
    ...
    for lig_file in ligand_files:
        ...
        if prot_file.exists():
            try:
                ...
                # 4. Evaluate Ranking (on ALL residues in the structure)
                if all_residues:
                    # Combine GT (min_dist) and Scores
                    system_data = []
                    ...
                    # For per-system print (fixed 6.0A)
                    ...
                    # Calculate Rank 3 Distance 4 Success explicitly
                    residue_data_4A = [(d < 4.0, s) for d, s in system_data]
                    residue_data_4A.sort(key=lambda x: x[1], reverse=True)
                    
                    best_rank_4A = float('inf')
                    for rank, (is_pocket, _) in enumerate(residue_data_4A, start=1):
                        if is_pocket:
                            best_rank_4A = rank
                            break
                    
                    success_rank3_dist4 = 1 if best_rank_4A <= 3 else 0
                    ...
                    residue_results.append({
                        ...
                        'Recall': case_r, 
                        'F1': case_f,
                        'BestPktRank': case_best_rank,
                        'Success_Rank3_Dist4': success_rank3_dist4
                    })

            except Exception as e:
                print(f"Residue eval failed for {system_id}: {e}")

        # 6. Pocket-level Metrics (Clusters)
    ...
    if all_system_residue_dist_scores:
        ...
        # Matrix to store success rates: [rank_k, dist_threshold]
        ...
        for d_idx, d_thresh in enumerate(dist_thresholds):
            ...
            for system_data in all_system_residue_dist_scores:
                # Sort residues by score desc
                sorted_res = sorted(system_data, key=lambda x: x[1], reverse=True)
                # Find best rank of a residue with dist < d_thresh
                best_r = float('inf')
            ...
            for k_idx, k in enumerate(rank_ks):
                success_count = sum(1 for r in system_ranks_at_d if r <= k)
                success_matrix[k_idx, d_idx] = (success_count / n_systems) * 100
        ...
        print(f"\nRANK SUCCESS RATES (%) at different distance thresholds:")
        ...
        # Show top systems by sorting the residue_results list
        sorted_results = sorted(residue_results, key=lambda x: x.get('BestPktRank', float('inf')))
        print("\nTOP 10 SYSTEMS BY BEST POCKET RANK:")
        print(f"{'SystemID':<35} {'Best Rank'}")
        for res in sorted_results[:10]:
            print(f"{res['SystemID']:<35} {res['BestPktRank']:.0f}")

    # Summary Pocket-level
    if all_dca_top1:
        ...
        plot_success_rates(cutoffs, dca_top1_rates, dca_top3_rates, 
                          'Success Rate (%)', 
                          base_dir / f'evaluate_residues2_{dataset_dir.name}_dca.svg')
        plot_success_rates(cutoffs, dcc_top1_rates, dcc_top3_rates, 
                          'Success Rate (%)', 
                          base_dir / f'evaluate_residues2_{dataset_dir.name}_dcc.svg')
    ...
    if all_dca_top1:
         # Save success rates summary table
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Evaluate YuelPocket2 residue and pocket predictions")
    parser.add_argument("dataset_folder", nargs="?", default="test50", help="Dataset folder")
