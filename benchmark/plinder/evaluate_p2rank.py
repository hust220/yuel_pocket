...
def get_ligand_coords(sdf_path):
    """Load ligand coordinates from SDF using RDkit."""
    try:
        ...
        coords = conf.GetPositions() # numpy array [N, 3]
        return coords
    except Exception as e:
        print(f"Error loading SDF {sdf_path}: {e}")
        return None
...
def evaluate_predictions(dataset_folder="test50"):
    ...
    ligand_files = list(dataset_dir.glob('*_ligand.sdf'))
    if not ligand_files:
    ...
    for lig_file in ligand_files:
        system_id = lig_file.name.replace('_ligand.sdf', '')
        ...
        detailed_results.append({
            ...
            'DCA_Top1': top1_dca,
            'DCC_Top1': top1_dcc,
            'DCA_Top3': min(top3_dcas),
            'DCC_Top3': min(top3_dccs),
            'Success_Top1_DCA_4': 1 if top1_dca < 4.0 else 0,
            'Success_Top1_DCC_4': 1 if top1_dcc < 4.0 else 0,
            'Success_Top3_DCA_4': 1 if min(top3_dcas) < 4.0 else 0,
            'Success_Top3_DCC_4': 1 if min(top3_dccs) < 4.0 else 0
        })
    ...
    if total_cases > 0:
        ...
        detailed_csv = base_dir / f'p2rank_{dataset_folder}_detailed.csv'
        fieldnames = ['SystemID', 'DCA_Top1', 'DCC_Top1', 'DCA_Top3', 'DCC_Top3', 
                      'Success_Top1_DCA_4', 'Success_Top1_DCC_4', 
                      'Success_Top3_DCA_4', 'Success_Top3_DCC_4']
